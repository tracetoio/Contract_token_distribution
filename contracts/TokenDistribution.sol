...
contract TokenDistribution is Ownable{
    ...
    event SetAllocation(address accAllocated, uint256 amount);
    event ContractAddressFound(address accAllocated, uint256 amount);
    ...
    function doAllocation(address _recipient, uint256 _amount, Mode _mode, bytes32 _comment) onlyOwner public returns(bool) {
        ...
        emit SetAllocation(_recipient, _amount);

        return true;
    }

    function doAllocationsWithSameAmount(address [] _recipients, uint256 _amount, Mode _mode, bytes32 _comment) onlyOwner public returns(bool) {
        if(_mode == Mode.AIRDROP){
            doAllocation(airdropWallet, _recipients.length.mul(_amount), _mode, _comment);
            ...
            for(uint8 idxAddr = 0; idxAddr < _recipients.length; idxAddr++){
                doAllocation(_recipients[idxAddr], _amount, _mode, _comment);
            }
        }
        return true;
    }

    function doAllocationsWithAmounts(address [] _recipients, uint256[] _amount, Mode _mode, bytes32 _comment) onlyOwner public returns(bool) {
        ...
        if(_mode == Mode.BOUNTY){
            ...
            doAllocation(bountyWallet, sum, _mode, _comment);
            t2tBounty.bounty(token, airdropWallet, _recipients, _amount);
        }
        else if(_mode == Mode.NORMAL || _mode == Mode.PREALLOCATED){
            for(uint8 idxAddr = 0; idxAddr < len; idxAddr++){
                doAllocation(_recipients[idxAddr], _amount[idxAddr], _mode, _comment);
            }
        }
        return true;
    }
